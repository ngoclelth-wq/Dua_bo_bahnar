...
export default function App() {
  const [questionsList, setQuestionsList] = useState<{ id?: string, q: string, a: number }[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetched = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as { id: string, q: string, a: number }[];
      
      if (fetched.length > 0) {
        setQuestionsList(fetched);
      } else {
        // If no questions in DB, use default ones
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {questionsList.map((q, idx) => (
                    <div key={q.id || idx} className="flex items-center justify-between p-4 bg-white border-2 border-gray-100 rounded-2xl hover:border-orange-200 transition-colors group">
